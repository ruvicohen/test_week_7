...
def get_missions_by_date_and_target_type(date, target_type):
    with session_maker() as session:
        return (session.query(func.avg(Mission.airborne_aircraft),
                              func.avg(Mission.attacking_aircraft),
                              func.avg(Mission.lost_aircraft))
                .join(Target, Mission.mission_id == Target.mission_id)
                .join(TargetType, TargetType.target_type_id == Target.target_type_id)
                .filter(Mission.mission_date == date)
                .filter(TargetType.target_type_name == target_type).scalar())
